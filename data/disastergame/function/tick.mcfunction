...
execute as @e[type=vindicator,tag=!jumping] at @s if entity @p[tag=alive,distance=4..6] run function disastergame:infectedlunge
execute as @e[type=vindicator,tag=jumping] at @s if entity @p[tag=alive,distance=..1] run data merge entity @s {Motion:[0.0, 0.0, 0.0]}
execute as @e[type=vindicator,tag=jumping] at @s if entity @p[tag=alive,distance=..1] run tag @s remove jumping
...
execute at @e[type=marker,tag=corruption,scores={timer=3..}] run summon chicken ~ ~ ~ {Silent:1b,Tags:["corruption"],active_effects:[{id:"minecraft:invisibility",amplifier:1,duration:10000,show_particles:0b}]}
scoreboard players reset @e[type=marker,tag=corruption,scores={timer=3..}] timer
...
execute at @e[type=phantom,tag=serpent,scores={timer=4..}] run summon block_display ~ ~ ~ {teleport_duration:5,Tags:["serpent"],transformation:{left_rotation:[0f,0f,0f,1f],right_rotation:[0.7071068f,0f,0f,0.7071068f],translation:[-1f,1f,-1f],scale:[2f,2f,2f]},block_state:{Name:"minecraft:fire"}}
